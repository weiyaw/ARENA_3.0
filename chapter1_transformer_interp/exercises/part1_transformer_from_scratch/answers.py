...
class LayerNorm(nn.Module):
    ...
    def forward(self, residual: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        residual = ((residual - mu) / t.sqrt(var + self.cfg.layer_norm_eps)) * self.w + self.b
        return residual


rand_float_test(LayerNorm, [2, 4, 768])
load_gpt2_test(LayerNorm, reference_gpt2.ln_final, cache["resid_post", 11])
tests.test_layer_norm_epsilon(LayerNorm, cache["resid_post", 11].cpu())
...
class PosEmbed(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_model"]:
        ...
        return einops.repeat(self.W_pos[:seq_len], "position d_model -> batch position d_model", batch=b)
...
class Attention(nn.Module):
    ...
    def forward(self, normalized_resid_pre: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        ...
        keys = einops.einsum(normalized_resid_pre, self.W_K, "b p d_model, h d_model d_head -> b p h d_head") + self.b_K
        queries = einops.einsum(normalized_resid_pre, self.W_Q, "b p d_model, h d_model d_head -> b p h d_head") + self.b_Q
        attn_scores = einops.einsum(queries, keys, "b q h d_head, b k h d_head -> b h q k") / (self.cfg.d_head ** 0.5)
        attn_scores = self.apply_causal_mask(attn_scores) # (b, h, q, k)
        attn_weights = t.softmax(attn_scores, dim=-1)


        values = einops.einsum(normalized_resid_pre, self.W_V, "b p d_model, h d_model d_head -> b p h d_head") + self.b_V
        z = einops.einsum(attn_weights, values, "b h q k, b k h d_head -> b h q d_head")
        output = einops.einsum(z, self.W_O, "b h q d_head, h d_head d_model -> b q h d_model")
        output = einops.reduce(output, "b q h d_model -> b q d_model", "sum") + self.b_O
        return output
...
class MLP(nn.Module):
    ...
    def forward(self, normalized_resid_mid: Float[Tensor, "batch posn d_model"]) -> Float[Tensor, "batch posn d_model"]:
        res = einops.einsum(normalized_resid_mid, self.W_in, "b p d_model, d_model d_mlp -> b p d_mlp") + self.b_in
        res = gelu_new(res)
        res = einops.einsum(res, self.W_out, "b p d_mlp, d_mlp d_model -> b p d_model") + self.b_out
        return res
...
class DemoTransformer(nn.Module):
    ...
    def forward(self, tokens: Int[Tensor, "batch position"]) -> Float[Tensor, "batch position d_vocab"]:
        x = self.embed(tokens) + self.pos_embed(tokens) # (b, p, d_model)
...
DemoTransformer(cfg).to(device)(tokens).shape
# %%
...
for i in range(10):
    ...
    # for j in demo_logits[-1]:
    #     print(reference_gpt2.tokenizer.decode(j.argmax()))
